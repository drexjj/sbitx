...
if [ -n "$pid" ]; then
    echo "$APP_NAME is already running with PID: $pid" >> /home/pi/x11vnc_sb_launcher.log
    ps -p $pid -o cmd= >> /home/pi/x11vnc_sb_launcher.log
    exit 0
fi
...
XVFB_PID=$!
echo "Xvfb PID: $XVFB_PID" >> /home/pi/x11vnc_sb_launcher.log
...
if netstat -tuln | grep -q :$VNC_PORT; then
    echo "Port $VNC_PORT is already in use, attempting to kill process" >> /home/pi/x11vnc_sb_launcher.log
    fuser -k $VNC_PORT/tcp
    sleep 1
fi

# Start x11vnc on our display, port $VNC_PORT
x11vnc -display :$DISPLAY_NUM -rfbport $VNC_PORT -rfbauth /home/pi/.vnc/passwd -shared -forever -o /home/pi/x11vnc_sb_launcher.log &
X11VNC_PID=$!
echo "x11vnc PID: $X11VNC_PID" >> /home/pi/x11vnc_sb_launcher.log
...
APP_PID=$!
echo "$APP_NAME PID: $APP_PID" >> /home/pi/x11vnc_sb_launcher.log

# Save PIDs for cleanup
echo "$XVFB_PID" > /tmp/sb_launcher_xvfb.pid
echo "$X11VNC_PID" > /tmp/sb_launcher_x11vnc.pid
echo "$APP_PID" > /tmp/sb_launcher_app.pid
