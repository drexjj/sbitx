...
if [ -f /tmp/sb_launcher_app.pid ]; then
    APP_PID=$(cat /tmp/sb_launcher_app.pid)
    ...
    rm -f /tmp/sb_launcher_app.pid
fi

# Stop x11vnc
if [ -f /tmp/sb_launcher_x11vnc.pid ]; then
    X11VNC_PID=$(cat /tmp/sb_launcher_x11vnc.pid)
    ...
    rm -f /tmp/sb_launcher_x11vnc.pid
fi

# Stop Xvfb
if [ -f /tmp/sb_launcher_xvfb.pid ]; then
    XVFB_PID=$(cat /tmp/sb_launcher_xvfb.pid)
