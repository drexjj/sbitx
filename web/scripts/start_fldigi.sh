...
if [ -n "$pid" ]; then
    echo "$APP_NAME is already running with PID: $pid" >> /home/pi/x11vnc_fldigi.log
...
if netstat -tuln | grep -q :$VNC_PORT; then
    echo "Port $VNC_PORT is already in use, attempting to kill process" >> /home/pi/x11vnc_fldigi.log
    fuser -k $VNC_PORT/tcp
    sleep 1
fi

# Start x11vnc on our display, port $VNC_PORT
x11vnc -display :$DISPLAY_NUM -rfbport $VNC_PORT -rfbauth /home/pi/.vnc/passwd -shared -forever -o /home/pi/x11vnc_fldigi.log &
X11VNC_PID=$!
...
APP_PID=$!
echo "$APP_NAME PID: $APP_PID" >> /home/pi/x11vnc_fldigi.log
