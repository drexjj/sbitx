...
if [ -f /tmp/app_name_app.pid ]; then
    APP_PID=$(cat /tmp/app_name_app.pid)
    ...
    rm -f /tmp/app_name_app.pid
fi

# Stop x11vnc
if [ -f /tmp/app_name_x11vnc.pid ]; then
    X11VNC_PID=$(cat /tmp/app_name_x11vnc.pid)
    ...
    rm -f /tmp/app_name_x11vnc.pid
fi

# Stop Xvfb
if [ -f /tmp/app_name_xvfb.pid ]; then
    XVFB_PID=$(cat /tmp/app_name_xvfb.pid)
