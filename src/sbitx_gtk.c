...
void draw_field(GtkWidget *widget, cairo_t *gfx, struct field *f)
{
	...
	case FIELD_BUTTON:
	{
		...
		else
		{
			...
			label_y = f->y + 4;
			draw_text(gfx, f->x + 4, label_y, label, FONT_FIELD_LABEL);
			width = measure_text(gfx, f->value, font_ix);
			label_y += font_table[FONT_FIELD_LABEL].height + 3;
			draw_text(gfx, f->x + 4, label_y, f->value,
					  font_ix);
		}
	}
	break;
...
int do_text(struct field *f, cairo_t *gfx, int event, int a, int b, int c)
{
	...
	else if (event == FIELD_DRAW)
	{
		fill_round_rect(gfx, f->x, f->y, f->width, f->height, 4, COLOR_CONTROL_BOX);
		if (f_focus == f)
			draw_round_rect(gfx, f->x, f->y, f->width, f->height, 4, COLOR_FIELD_SELECTED);
