...
//rx_linear with Spectral Subtraction and Weiner Filter DSP filtering - W2JON
    ...
    int32_t *output_speaker, int32_t *output_tx, int n_samples)
{
    ...
    if (r->mode != MODE_DIGITAL && r->mode != MODE_FT8 && r->mode != MODE_2TONE) {
        ...
        if (anr_enabled) {
           ...
           // Automatic Frequency Peaking Filter (APF)
          if (apf_enabled && (r->mode == MODE_CW || r->mode == MODE_CWR)) {
                double center_frequency = rx_pitch;
                double bandwidth = apf_bandwidth;
                double half_bandwidth = bandwidth / 2.0;

            for (i = 0; i < MAX_BINS; i++) {
          // Calculate the frequency for the current bin
                double frequency = (i - MAX_BINS / 2) * (sampling_rate / MAX_BINS);
        
          // Calculate distance from the center frequency
                double distance = fabs(frequency - center_frequency);
        
          // Apply different logic for CW and CWR modes
          if (r->mode == MODE_CW) {
            // CW mode: Amplify frequencies within the band centered around the center frequency
             if (distance <= half_bandwidth) {
                 r->fft_freq[i] *= 2.0; // Amplify frequencies within the band
             } else {
                 r->fft_freq[i] *= 0.1; // Attenuate frequencies outside the band
             }
         } else if (r->mode == MODE_CWR) {
             // CWR mode: Flip the filter logic to handle frequencies below the center frequency
             if ((frequency <= center_frequency) && (center_frequency - frequency <= half_bandwidth)) {
                 r->fft_freq[i] *= 2.0; // Amplify frequencies within the band
             } else {
                 r->fft_freq[i] *= 0.1; // Attenuate frequencies outside the band
             }
